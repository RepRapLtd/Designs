...
int pSetting;
int pReading;
double pReadingMod = 0.9;
int pReadingModAdd = 100;
char slaveSignal;
int loopTiming = 500;
int pTransmit;
...
void inputPrompt(){
    lineBreak(1);
    Serial.print("Please enter an input between ");
    Serial.print(pMin);
    Serial.print(" and ");
    Serial.print(pMax);
...
void inputOpen(){
  Serial.print("Ready for pressure setting. "); 
  inputPrompt();
  //if(!Serial.available());
...
void inputCheck(){
  ...
  if (pSettingNew <= pMax && pSettingNew >= pMin){ // Check input is within pressure setting range
    ...
    else {Serial.print("Invalid pressure input! "); inputPrompt();  } // Tell user to use valid input
    }
    } 
}
